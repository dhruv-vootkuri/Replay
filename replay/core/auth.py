...
REPLAY_API_KEY to one from Floe's dashboard (Settings > API Keys). Uses only
...
class InvalidApiKey(Exception):
    """Raised when REPLAY_API_KEY is missing or Floe rejects it."""
...
def verify_api_key(api_key: str = None, floe_url: str = None) -> None:
    ...
    """
    api_key = api_key or os.environ.get("REPLAY_API_KEY")
    floe_url = floe_url or os.environ.get("FLOE_API_URL", DEFAULT_FLOE_URL)

    if not api_key:
        raise InvalidApiKey(
            "No API key configured. Set REPLAY_API_KEY to a key from your "
