...
class ReplayEngine:
    ...
    def replay(
        ...
        Args:
            ...
            meta:          Extra fields stored on the saved replay — where it
                           came from ("fork", "pressure"), which run it belongs
                           to, a human label.  Shows up in the replay log.
        """
        ...
        result = {
            ...
            "changes_readable": self._readable_changes(fork_span, changes),
    ...
    def _readable_changes(
        self, fork_span: Dict[str, Any], changes: Dict[str, Any]
    ) -> list:
        """
        Compares each change against the value it replaced and returns only the
        fields that actually differ, as {field, before, after}.  Stored on the
        replay so the log and the diff view don't have to re-derive it.

        `fork_span` must be the pre-fork span — the engine deep-copies before
        applying changes, so the original dict is still untouched here.
        """
        attrs = fork_span.get("attributes", {})
        readable = []

        for key, new_val in (changes or {}).items():
            if key == "replay.messages_json":
                try:
                    new_msgs = json.loads(new_val)
                except (json.JSONDecodeError, TypeError):
                    continue
                old_raw = attrs.get("replay.messages_json")
                try:
                    old_msgs = json.loads(old_raw) if old_raw else []
                except (json.JSONDecodeError, TypeError):
                    old_msgs = []

                # A system message added where there was none shifts every
                # later index — report the addition, then compare the rest in
                # its own frame instead of flagging the whole history.
                added_system = (
                    len(new_msgs) == len(old_msgs) + 1
                    and new_msgs
                    and new_msgs[0].get("role") == "system"
                    and (not old_msgs or old_msgs[0].get("role") != "system")
                )
                if added_system:
                    readable.append({
                        "field": "system",
                        "before": "",
                        "after": str(new_msgs[0].get("content", "") or ""),
                    })
                    new_msgs = new_msgs[1:]

                for i, msg in enumerate(new_msgs):
                    before = (
                        str(old_msgs[i].get("content", "") or "")
                        if i < len(old_msgs) else ""
                    )
                    after = str(msg.get("content", "") or "")
                    if before != after:
                        readable.append({
                ...
                old_args = self._extract_tool_args(attrs)
                try:
                ...
                before = str(attrs.get(key, ""))
                if before != str(new_val):
                    readable.append({
                        "field": key,
                        "before": before,
                        "after": str(new_val),
    ...
    def _generate_summary(
        ...
        readable_changes = [
            f"{change['field']}='{change['after'][:30]}'"
            for change in self._readable_changes(fork_span, changes)
            if change["field"] not in ("gen_ai.tool.call.result", "replay.tool_result")
        ]
